...
main_estimates$cohort <- factor(main_estimates$cohort, levels=c("prevax","vaccinated","electively_unvaccinated")) 
main_estimates$colour <- factor(main_estimates$colour, levels=c("#d2ac47","#58764c","#0018a8"))

# Rename adjustment groups
levels(main_estimates$cohort) <- list("Pre-vaccinated (2020-01-01 - 2021-06-18)"="prevax", "Vaccinated (2021-06-01 - 2021-12-14)"="vaccinated","Electively unvaccinated (2021-06-01 - 2021-12-14)"="electively_unvaccinated")
...
df <- main_estimates
df$outcome <- str_to_title(df$outcome)

# force median follow up for now

df <- df %>%
  mutate(median_follow_up = ifelse(term == "days0_28", 2,
                                   ifelse(term == "days28_197", 12,
                                          ifelse(term == "days197_535", 36, 0))))
