...
stage1 <- function(cohort_name, group){
  ...
  write.csv(QA_summary, file = file.path("output/review/descriptives", paste0("explore_QA_summary_",cohort_name, "_",group, ".csv")) , row.names=F)
  ...
  sink(file = file.path("output/not-for-review", paste0("explore_meta_data_factors_",cohort_name, ".csv")))
  print(meta_data_factors)
  sink()
  #----------------------#
  # 3.d. Create csv file #
  #----------------------#
  write.csv(cohort_flow, file = file.path("output/review/descriptives", paste0("explore_Cohort_flow_",cohort_name, "_",group, ".csv")) , row.names=F)
  ...
  svglite::svglite(file = file.path("output/not-for-review/", paste0("explore_numeric_histograms_", cohort_name, "_", group, ".svg")), width = 15, height = 8)
  ...
  # Create unvax sensitivity analysis columns
  ...
  saveRDS(input, file = file.path("output", paste0("explore_input_",cohort_name, "_stage1_",group,".rds")))
  
  # Save as CSV as well only for diabetes group to save time
  
  if (group == "diabetes"){
  readr::write_csv(input, paste0("output/explore_input_",cohort_name, "_stage1_",group,".csv.gz"))
  }
}
