...
# exclude main exposures - but keep t2dm so we can co
outcomes_to_plot <- c("t2dm", "t2dm_pre_rec", "t2dm_post_rec")
#---------------------------------------------#
...
main_estimates <- estimates %>% filter(subgroup == "covid_pheno_hospitalised" 
...
main_estimates$outcome[main_estimates$outcome == 'Type 2 Diabetes - Pre_recovery'] <- 'Type 2 diabetes - Pre RECOVERY'
main_estimates$outcome[main_estimates$outcome == 'Type 2 Diabetes - Post_recovery'] <- 'Type 2 diabetes - Post RECOVERY'


main_estimates$outcome <- factor(main_estimates$outcome, levels=c("Type 2 diabetes - Main analysis",
                                                                  "Type 2 diabetes - Pre RECOVERY",
                                                                  "Type 2 diabetes - Post RECOVERY"))
...
for(i in c("any_position")){
  ...
  ggplot2::ggplot(data=df,
    ...
    ggplot2::facet_wrap(outcome~., ncol = 3)
  
  ggplot2::ggsave(paste0(output_dir,"t2dm_pre-post-recovery",i,".png"), height = 180, width = 220, unit = "mm", dpi = 600, scale = 1)
