...
#-------------------------#
main_figures_1 <- function(cohort, time_point) {
  ...
  # subset to just the two models if they are looking OK. If not, subset as appropriate.
  main_estimates <- main_estimates %>%
    dplyr::filter(model == "mdl_age_sex" | model == "mdl_max_adj")
  ...
  print("Uses mdl_agesex and mdl_max_adj by default. If you wish to plot different models, change line 76")
  # Rename adjustment groups
  levels(main_estimates$model) <- list("Age/sex/region"="mdl_age_sex", "Extensive"="mdl_max_adj")
  ...
  if(time_point == "reduced"){
    ...
    for(c in cohort){
      for (o in unique(main_estimates_reduced$event)) {
        df=main_estimates_reduced %>% filter(cohort == c & event == o)
        ...
        p <- ggplot2::ggplot(data=df,
          ...
          # ggtitle(paste0(str_to_title(df$outcome), " ", str_to_title(cohort))) +
    ...
    main_estimates_normal <- subset(main_estimates, time_points == "normal")
    
    plot_list <- list()
    
    for(c in cohort){
      for (o in unique(main_estimates_normal$event)) {
        df=main_estimates_normal %>% filter(cohort == c & event == o)
