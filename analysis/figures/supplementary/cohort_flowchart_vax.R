...
values <- read.csv(paste0(results_dir,"/","Cohort_flow_vax_diabetes.csv"))
# calculate N removed with diabetes
values[17,2] <- values[16,1] - values[17,1]
...
criteria_8 <- boxGrob(glue("Criteria 8 (Exclusion): No record of two vaccination doses prior to the study end date",
                           "N = {pop}",
...
criteria_9 <- boxGrob(glue("Criteria 9 (Exclusion): Recorded vaccination prior to the start date of vaccination program",
                           "N = {pop}",
...
criteria_10 <- boxGrob(glue("Criteria 10 (Exclusion): Second dose vaccination recorded before the first dose vaccination",
                           "N = {pop}",
...
criteria_11 <- boxGrob(glue("Criteria 11 (Exclusion): Second dose vaccination recorded less than three weeks after the first dose",
                            "N = {pop}",
...
criteria_12 <- boxGrob(glue("Criteria 12 (Exclusion): Received mixed vaccine products before 07-05-2021",
                            "N = {pop}",
...
criteria_13 <- boxGrob(glue("Criteria 13 (Inclusion): Patient index date is within the study start and end dates i.e patient is fully vaccinated before the study end date",
                            "N = {pop}",
                            pop = txtInt(values[16,1]),
                            .sep = "\n"))
...
exclude15 <- boxGrob(glue("Excluded (n = {tot})",
                          tot = txtInt(values[16,2]),
                          .sep = "\n"),
                     just = "left")

exclude16 <- boxGrob(glue("Excluded (n = {tot})",
                          tot = txtInt(values[17,2]),
                          .sep = "\n"),
                     just = "left")
...
exclude13 <- moveBox(exclude13,
                     x = .8,
                     y = coords(vert$criteria_11)$top + distance(vert$criteria_10, vert$criteria_11, half = TRUE, center = FALSE))

exclude14 <- moveBox(exclude14,
                     x = .8,
                     y = coords(vert$criteria_12)$top + distance(vert$criteria_11, vert$criteria_12, half = TRUE, center = FALSE))

exclude15 <- moveBox(exclude15,
                     x = .8,
                     y = coords(vert$criteria_13)$top + distance(vert$criteria_12, vert$criteria_13, half = TRUE, center = FALSE))

exclude16 <- moveBox(exclude16,
                     x = .8,
