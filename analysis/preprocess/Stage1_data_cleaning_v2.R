...
stage1 <- function(cohort_name, group){
  ...
  write.csv(QA_summary, file = file.path("output/review/descriptives", paste0("QA_summary_",cohort_name, "_",group, "_v2.csv")) , row.names=F)
  ...
  sink(file = file.path("output/not-for-review", paste0("meta_data_factors_",cohort_name, "_v2.csv")))
  print(meta_data_factors)
  sink()
  #----------------------#
  # 3.d. Create csv file #
  #----------------------#
  write.csv(cohort_flow, file = file.path("output/review/descriptives", paste0("Cohort_flow_",cohort_name, "_",group, "_v2.csv")) , row.names=F)
  ...
  svglite::svglite(file = file.path("output/not-for-review/", paste0("numeric_histograms_", cohort_name, "_", group, "_v2.svg")), width = 15, height = 8)
  ...
  saveRDS(input, file = file.path("output", paste0("input_",cohort_name, "_stage1_",group,"_v2.rds")))
  
  # Save as CSV as well only for diabetes group to save time
  
  if (group == "diabetes"){
  readr::write_csv(input, paste0("output/input_",cohort_name, "_stage1_",group,"_v2.csv.gz"))
  }
}
